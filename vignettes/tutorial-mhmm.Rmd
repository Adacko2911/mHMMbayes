...
To specify user specific prior distributions, one uses the input option `emiss_hyp_prior` for the emission distribution and `gamma_hyp_prior` for the transition probabilities in the function `mHMM`. Both objects are a list, containing the elements:

 * `mu0`, a list containing $m$ matrices; one matrix for each row of the transition probability matrix gamma or the emission probability matrix. Each matrix contains the hypothesized mean values of the intercepts. Hence, each matrix consists of one row (when not including covariates in the model) and $m - 1$ or $q$ - 1 columns.
 ...
 * `V`, the hypothesized variance-covariance matrix between the set of intercepts.  

Note that `K0`, `nu` and `V` are assumed equal over the states. The mean values of the intercepts (and regression coefficients of the covariates) denoted by `mu0` are allowed to vary over the states. All elements in the list either have the prefix `gamma_` or `emiss_`, depending on which list they belong to. When specifying prior distributions, note that the first element of each row does not have an intercept, as it serves as baseline category in the multivariate regression model. This means, for example, that if we would specify a model with 3 states, `mu0` is a vector with 2 elements, `K0` and `nu` contain 1 element and `V` is a 2 by 2 matrix.  
...
```{r show obtain emiss function}

# When not specified, level defaults again to "group"
emiss_pop <- obtain_emiss(out_2st)
emiss_pop
# Subject specific parameter estimates:
emiss_subj <- obtain_emiss(out_2st, level = "subject")
emiss_subj
```
...
Lastly, user can also plot the posterior emission probability distribution related to each state of the model. As an input we use objects coming from `obtain_emiss` function. The plot function plots by default the emission probability distributions of dependent variables of categorical data as grouped by dependent variables. To plot the distributions by states, the `by_state` argument has to be set to true. Note that when plotting continuously distributed emission probabilities the function plots only variables grouped by dependent variables. 

```{r show barplots of emission probability distribution}

#Emission probability distribution at the group level displayed by states  
plot(emiss_pop)

#Emission probability distribution for subject number 2 displayed by dependent variable
plot(emiss_subj,subj_nr = 2,by_state=F)

```
When plotting the subject level emission probabilities the subject number `subj_nr` need to be specified within the arguments of the `plot()` function. This function allows for some further customization. One can check that by running the help file of its help file `?plot.mHMM_emiss`.
...
### Simulating data 
The package supply users with a possibility to simulate data of multilevel structure that follows a hidden Markov model providing `sim_mHMM()` function. 
One can simulate specific number of observations `n_t` for specified number of subjects `n` that follows a model with `m` hidden states of a dependent variable with `q_emiss` categorical outcome. To do that the transition probability matrix for given number of states and the emission probability distribution matrix has to be provided in respectively `gamma` and `emiss_distr` arguments.  
 
Here we can see an example of simulating data for $10$ subjects with each $100$ observations for `m=3` state hidden Markov Model (HMM). This is done for one dependent variable with $4$ categorical outcomes with average emission probability distribution matrix `emiss_distr`. 

```{r simulating states and observations, eval=T }
n_t     <- 100
n       <- 10
m       <- 3
q_emiss <- 4

#A matrix containing the average population transition probability matrix 
...
```
It is possible to specify `var_gamma` and/or `var_emiss` representing respectively variance between subjects in transition probability matrix and variance between subjects in the emission distribution(s). The default of both variables is set to $0.1$ corresponding to little variation between subjects.

the transition probability and/or emission distribution matrices for `n` number of subjects (we obtain the simulation only by setting `return_ind_par=TRUE`). 

The function allows for including covariates matrices for simulating gamma and/or emission probability distribution matrices. We do that by simply including list of covariance matrices `xx_vec` with the first element on the list representing covariance matrix for transition probability matrix and rest representing covariance for each of dependent variables. When specifying covariates user need to include a list of the regression parameters `beta` with  to predict gamma and/or emission distribution in combination with `xx_vec` using (multinomial logistic) regression.
To simulate only emission or transition matrix using covariates we include them and betain positions corresponding to desirable in `xx_vec` and `beta` lists. The rest of inputs that we are not interested in should be set to $0$. 
For coding covariance for continuous data see example below or type '?sim_mHMM' in console. 
 
Here we simulate data including a covariate to predict (only) the transition probability matrix (*Note that it is continuation of example above with univariate cathegorical data*): 
```{r including a covariate to predict (only) the transition probability matrix, eval=TRUE}
...
data2 <- sim_mHMM(n_t = n_t, n = n, m = m, q_emiss = q_emiss, gamma = gamma,
                  ...
                  var_gamma = 1, var_emiss = 1,return_ind_par=TRUE) 

data2$subject_gamma

```

Command *data2$subject_gamma* returns a list containing $n$ elements with the simulated subject specific transition probability matrices gamma.


Lastly, you could also account for continuous distribution of multiple dependent variables. 
In the example below `n_dep`(representing number of dependent variables) is set to $2$. We know that dependent variables are continuous and so the matrices we plug to `emiss_distr`(average population emission distributions for each of the dependent variables) are matrices of $m$ rows and $2$ columns(in example we perform simulation for model with `m=2` hidden states). The structure is that in first column in each row i we got the mean of each state i and in the second column includes in each row i the variance of state i of the Normal distribution. We also need to set the observation type of the data `data_distr = 'continuous'` in `sim_mHMM` function.
```{r simulating multivariate continuous data, eval=F}
n_t     <- 100
n       <- 10
m       <- 3
n_dep   <- 2
...
head(data_cont$obs)
```
The output is the first 6 rows of simulated observations and the first 6 rows of simulated states. You could also call *data_cont$subject_gamma* or *data_cont$emiss_distr* to check simulated subject specific transition or emission probability. However, remember that these will be returned only if `return_ind_par` is set to `TRUE` in `sim_mHMM` function.


Another feature is that the data can be simulated, for `n_dep` dependent variables which distribution can be either 'categorical' or 'continuous'(we specify it in `data_distr` variable). 
